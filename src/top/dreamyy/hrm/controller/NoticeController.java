...
public class NoticeController {
	 ...
	 * 处理/login请求
	 * */
	@RequestMapping(value="/notice/selectNoticeAdvice")
	 public String selectNoticeAdvice(Model model,Integer pageIndex,
			 @ModelAttribute Notice notice){
		PageModel pageModel = new PageModel();
		if(pageIndex != null){
			pageModel.setPageIndex(pageIndex);
		...
		/** 查询用户信息     */
		List<Notice> notices = (List<Notice>) hrmService.findNoticeByIdAdvice(1);
		model.addAttribute("notices", notices);
		model.addAttribute("pageModel", pageModel);
		return "notice/noticeAdvice";
		
	}
	
	
	
	@RequestMapping(value="/notice/selectNoticeUser")
	 public String selectNoticeUser(Model model,Integer pageIndex,
			 @ModelAttribute Notice notice){
		PageModel pageModel = new PageModel();
		if(pageIndex != null){
			pageModel.setPageIndex(pageIndex);
	...
	@RequestMapping(value="/notice/addNotice")
	 public ModelAndView addNotice(
		...
		if(flag.equals("1")){
			...
			if(type != null) {
				notice.setStatus(Integer.parseInt(type));
			}
			notice.setUser(user);
	...
	@RequestMapping(value="/notice/addNoticeAdvice")
	 public ModelAndView addNoticeAdvice(
			 String type,
			 String flag,
			 @ModelAttribute Notice notice,
			 ModelAndView mv,
			 HttpSession session){
		if(flag.equals("1")){
			mv.setViewName("notice/showAddNoticeAdvice");
		}else{
			User user = (User) session.getAttribute(HrmConstants.USER_SESSION);
			if(type != null) {
				notice.setStatus(Integer.parseInt(type));
			...
			notice.setUser(user);
			hrmService.addNotice(notice);
			mv.setViewName("redirect:/notice/selectNoticeAdvice");
		}
		// 返回
		return mv;
	}
