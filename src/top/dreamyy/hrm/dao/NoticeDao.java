...
import static top.dreamyy.hrm.util.common.HrmConstants.NOTICETABLEUSER;
...
public interface NoticeDao {
	...
	@Select("select * from "+NOTICETABLE+" where status = #{status}")
	List<Notice> selectByIdAdvice(int status);
	
	@Select("select * from "+NOTICETABLEUSER+" where ID = #{id}")
	Notice selectByIdUser(int id);
